...
```
devcontainer-home-hermes-agent-001  (external Docker volume)
    └── /home/vscode/               （~/.hermes, ~/.local, ~/.bashrc 等）

/workspaces/hermes-agent-001/        （ホストバインドマウント、Git管理）
    ├── .devcontainer/
    │   ├── devcontainer.json
    │   ├── docker-compose.yml
    │   ├── Dockerfile
...
```bash
git clone https://github.com/shichiyou/hermes-agent-001.git
cd hermes-agent-001
...
#   例: export WIKI_PATH="/workspaces/hermes-agent-001/wiki"

# 3-6: Wiki シンボリックリンクの確認
ls -la ~/wiki
# 期待結果: /workspaces/hermes-agent-001/wiki へのシンボリックリンク
...
# Docker ボリュームの削除
docker volume rm devcontainer-home-hermes-agent-001
# 期待結果: ボリュームが削除されたこと
