...
§
Biome: auto-gen files (graphify-out/) excluded via biome.json includes negation, not formatted. Schema mismatch = info→patch $schema. root-owned files need sudo. VS Code "undefined" status bar: ~ (Home) folder has no biome binary→add `"[/home/vscode]":{"biome.enabled":false}` in .code-workspace settings. User keeps multi-root structure.