...
### ピットフォール: 「CI側に手足があるが、AI-DLCルール側に脳がない」
...
以下の3変更は2025-04-27に `experiences/aidlc-codd-graphify-lab` で実装済み。AGENTS.md, CLAUDE.md 両方に同一変更を適用し、物理的検証も完了。

**変更1**: Build and Test ステージに CoDD/Graphify 実行ゲート追加（実装済み）

AGENTS.md L420-436 に実装。Build and Test の手順3として決定的ゲートを追加:

```markdown
3. **EXECUTE deterministic gates before generating instructions**:
   - Run `python -m pytest -v sample-app/tests`
     - IF FAIL: enter repair loop (fix → rerun, max 3 attempts)
     - IF all attempts FAIL: report blocker in audit.md, wait for human
     - IF PASS: record result in audit.md, continue
   - Run `codd validate && codd scan && codd measure`
     - Record coverage metrics in audit.md
   - Run `python scripts/check_traceability_coverage.py`
     - IF FAIL: report blocker in audit.md, wait for human intervention
     - IF PASS: record result in audit.md, continue
   - Run `graphify update sample-app`
     - Record graph metrics in audit.md
```

効果: Build and Test の人間承認が「指示文レビュー」から「実行結果レビュー」に変わる。承認プロンプトも「Build and test instructions complete」→「Build and test complete with gate results above」に変更。

**変更2**: Code Generation パート2にコード検証ステップ追加（実装済み）

AGENTS.md L403-412 に実装。Code Generation の手順5として実行検証を追加:

```markdown
5. **EXECUTE post-generation verification**:
   - Run `python -m pytest -v sample-app/tests`
     - IF FAIL: auto-repair within this unit's scope (max 3 attempts)
     - IF all attempts FAIL: report blocker in audit.md, mark in completion message
     - IF PASS: record as code generation verification evidence in audit.md
   - Run `codd validate && codd scan`
     - Record coherence state in audit.md
```

効果: コード生成直後に自動テストが走る。FAILなら修復ループ。これが「Bolt内の自動検証」の実装相当。

**変更3**: `.aidlc-rule-details/` の旧版修正（実装済み）

`workflow-planning.md` と `units-generation.md` をアップストリーム最新版で上書き済み。PR #156 マージ前の「Units Planning」ゴーストステージを削除:

```bash
curl -sL -o .aidlc-rule-details/inception/workflow-planning.md \
  https://raw.githubusercontent.com/awslabs/aidlc-workflows/main/aidlc-rules/aws-aidlc-rule-details/inception/workflow-planning.md
curl -sL -o .aidlc-rule-details/inception/units-generation.md \
  https://raw.githubusercontent.com/awslabs/aidlc-workflows/main/aidlc-rules/aws-aidlc-rule-details/inception/units-generation.md
```

物理的検証: `diff <(curl upstream) <local>` → IDENTICAL、`grep -c "Units Planning"` → 0件。
...
1. AGENTS.md と CLAUDE.md の該当箇所（Build and Test セクション、Code Generation セクション）に上記の決定的ゲートと検証ステップを `patch` で挿入する。
2. `.aidlc-rule-details/inception/workflow-planning.md` と `units-generation.md` がアップストリーム最新版と同一か `diff` で確認し、異なれば上書きする。
3. `diff AGENTS.md CLAUDE.md` で両者が同一であることを検証する。
4. `grep -nc 'codd\|graphify\|pytest' AGENTS.md` で参照数が0でないことを確認する。
