...
## AI-DLC の定義範囲（どこまで定義されているか）

AI-DLC は「ユーザー要求の受け取りからビルド＆テスト指示文書生成まで」を定義している。リリース以降はプレースホルダのみ。

| フェーズ | 定義状態 | 詳細 |
|---|---|---|
| INCEPTION 全ステージ | ルール詳細ファイルあり | Workspace Detection (97行), Reverse Engineering, Requirements Analysis (189行), User Stories, Workflow Planning (469行), Application Design, Units Generation |
| CONSTRUCTION: 設計系 | ルール詳細ファイルあり | Functional Design, NFR Requirements, NFR Design, Infrastructure Design |
| CONSTRUCTION: Code Generation | ルール詳細ファイルあり（217行） | Part 1 Planning + Part 2 Generation |
| CONSTRUCTION: Build and Test | テンプレートのみ（360行） | **実際のテスト実行コマンドは定義されていない**。ビルド指示・テスト指示のテンプレートを生成する手順書 |
| OPERATIONS | プレースホルダのみ（19行） | "将来的にDeployment/Monitoring/Incident含む"の記述のみ |

重要な区別:
- `build-and-test.md` は「テストを実行するコマンド」ではなく「テスト指示文書を生成するテンプレート」である
- テスト実行の具体的コマンド（pytest, codd, graphify等）はプロジェクトごとにLAB-SPECIFIC区域または `docs/process/` で定義する必要がある
- CoDD, Graphify, traceability check はAI-DLC公式配布物に含まれない。`.aidlc-rule-details/` 29ファイル中これらの言及はゼロ件
- OPERATIONS フェーズは「placeholder for future expansion」と明記されており、デプロイ・監視・保守・変更管理の運用化はプロジェクト側で補完する必要がある
...
```bash
# 3ファイル間の同期確認（コマンドが0件なら役割定義セクションが欠落）
grep -c '役割分担\|合否判定\|INFERRED\|structural verification' AGENTS.md CLAUDE.md .github/copilot-instructions.md

# 3ファイル間の同一性確認（AGENTS.md と CLAUDE.md は同一であるべき）
diff AGENTS.md CLAUDE.md | wc -l

# 3ファイル間のツール言及数（0または著しく異なる場合は同期欠落）
grep -c 'graphify\|Graphify' AGENTS.md CLAUDE.md .github/copilot-instructions.md
grep -c 'codd\|CoDD' AGENTS.md CLAUDE.md .github/copilot-instructions.md

# Build and Test ゲートの存在確認（copilot-instructions.md でよく欠落する）
grep -c 'deterministic gates' .github/copilot-instructions.md
# 結果が0なら、Build and Test の決定的ゲートが欠落している
```
...
## AI-DLC × CoDD × Graphify ラボからテンプレートを抽出する場合

実験ラボをそのまま他プロジェクトの雛形にしない。ラボは Raw Output、失敗ログ、生成物、教育用演習を保持する場所であり、テンプレートは再利用可能な profile だけを抽出した独立リポジトリにする。

推奨 profile:

| プロファイル | 内容 |
|---|---|
| Core | AI-DLC指示、`.aidlc-rule-details/`、CoDD設定、process docs、CI/pre-commit、検証スクリプト |
| Python | pytest で動く最小サンプル |
| Web Design | `DESIGN.md` と design token 例 |
| Evidence Hardening | Evidence Matrix / 初回検証ログのテンプレート |
| Lab/Tutorial | HANDS_ON、実験ログ、Before/After 演習。通常テンプレートからは分離 |

テンプレートに含めないもの:

```text
logs/
aidlc-docs/evidence/*.log
experiments/
codd/scan/
...
テンプレートには `template-manifest.yaml` を置き、AI-DLC release、取得元、sha256、CoDD/Graphifyの観測バージョン、profile一覧を記録する。導入スクリプトは既存ファイルを上書きせず停止させ、smoke test で instantiate 結果と最小テストを確認してから GitHub repo / submodule 化する。

実行手順の詳細は `workspace-hygiene-and-git-discipline` の "Pattern C: Extracting a Clean Template Repo from an Experimental Lab" を参照。

## GitHub Template Repository 化する場合の必須検証

AI-DLC × CoDD × Graphify のラボ成果を GitHub Template Repository として切り出す場合、`profiles/` に部品を置くだけでは不十分。GitHub の `Use this template` は repository root を新規 repository の root として複製するため、root 自体が即起動可能な starter project でなければならない。

### 必須の構造

GitHub template として成立する最小 root 構造:
...
`profiles/` は補助・再展開用として残してよいが、AIエージェントが最初に読む `AGENTS.md` / `.aidlc-rule-details/` / `codd/codd.yaml` が root にない状態は「即起動可能テンプレート」ではない。

### Assumption Surfacing の適用

ユーザーが「テンプレートリポジトリを作って」「具体の状態を作って」と依頼した場合、次の前提を暗黙に決めてはいけない。

質問または仮説提示を行うこと:

```text
GitHub Template Repository として、root をそのまま新規リポジトリに複製して即 AI-DLC を開始できる starter project を想定しますか？
それとも profiles/ 配下の部品を展開する toolkit を想定しますか？
推奨は root starter + profiles 補助の両立です。
```

これを怠ると「見た目はテンプレートだが、エージェントを起動して要求定義を開始できない」構造になる。

### 必須の物理検証

GitHub Template Repository としての完了主張は、実際に template 生成した新規 repository で検証するまで禁止。

検証手順:

```bash
# 1. Template repository 設定確認。false なら有効化する。
gh repo view OWNER/TEMPLATE --json nameWithOwner,isTemplate,isPrivate,defaultBranchRef
# 有効化が必要な場合:
gh api -X PATCH repos/OWNER/TEMPLATE -f is_template=true --jq '{full_name,is_template,private,html_url}'

# 2. GitHub template generation API で検証用 repository を作る。
# gh repo create --template は環境・バージョンにより空 repository を作る挙動を観測したため、API generate を正とする。
SMOKE_REPO="template-smoke-$(date -u +%Y%m%d%H%M%S)"
gh api -X POST repos/OWNER/TEMPLATE/generate \
  -f owner=OWNER \
  -f name="$SMOKE_REPO" \
  -F private=true \
  -F include_all_branches=false \
  --jq '{full_name,html_url,private,default_branch}'

# 3. default branch が作られるまで待って clone。
gh repo view OWNER/$SMOKE_REPO --json defaultBranchRef

gh repo clone OWNER/$SMOKE_REPO /tmp/$SMOKE_REPO
cd /tmp/$SMOKE_REPO

# 4. root 即起動条件を確認。
for p in AGENTS.md CLAUDE.md .github/copilot-instructions.md \
  .aidlc-rule-details/common/process-overview.md codd/codd.yaml \
  docs/requirements/requirements.md aidlc-docs/aidlc-state.md aidlc-docs/audit.md \
  sample-app/pyproject.toml scripts/check_traceability_coverage.py scripts/check_graphify_coverage.py; do
  test -e "$p" && echo "FOUND $p" || echo "MISSING $p"
done

# 5. 実ゲートを生成先 repository root で実行。
python -m pytest -v sample-app/tests
codd validate
codd scan
codd measure
...
合格条件:
- `isTemplate: true`。
- 生成先 repository が空ではない。
- 生成先 root に AI-DLC / CoDD / Graphify の必須ファイルがある。
- pytest / CoDD / traceability / Graphify coverage が PASS。
...
方針:
- `**/graphify-out/` を `.gitignore` に入れる。
- Graphify 出力は検証時に生成する。
- `scripts/check_graphify_coverage.py` は生成済み `graph.json` を読む。
- 固定の旧サンプルパス（例: `sample-app/src/taskflow/*.py`）に依存せず、現在の `sample-app/src/**/*.py` と `sample-app/tests/**/*.py` から期待ファイルを動的発見する。

### 既知の CLI ピットフォール

`gh repo create --template OWNER/TEMPLATE` で検証用 repository を作った際、空 repository が作成され `warning: You appear to have cloned an empty repository.` / `defaultBranchRef.name == ""` となる挙動を観測した。GitHub UI の `Use this template` 相当の物理検証には、GitHub API の `/repos/{template_owner}/{template_repo}/generate` を使う。

検証用 repository の削除は破壊的操作なので、ユーザー承認なしに削除しない。
